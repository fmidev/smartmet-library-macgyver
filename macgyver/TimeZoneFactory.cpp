...
vector<string> TimeZoneFactory::region_list()
{
  try
  {
    return TimeZonePtr::get_region_list();
  }
  catch (...)
  {
    throw Fmi::Exception::Trace(BCP, "Operation failed!");
...
Fmi::TimeZonePtr TimeZoneFactory::time_zone_from_string(const string& desc)
{
  try
  {
    // FIXME: It looks that custom time zones are not supported in data library (or std::chrono)
    //        So act as time_zone_from_region() for now
    Fmi::TimeZonePtr ptr(desc);
    return ptr;
  }
  catch (...)
  {
