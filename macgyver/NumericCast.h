...
numeric_cast(Source value)
{
  ...
  try
  {
    ...
    else if constexpr (std::is_signed_v<Source> && std::is_unsigned_v<Target>)
    {
      ...
      {
        throw Fmi::Exception(
            BCP, make_range_error(value, target_min, target_max, "unsigned target type"));
      }
    }
    ...
    else
    {
      ...
      {
        throw Fmi::Exception(BCP, make_range_error(value, target_min, target_max, "target type"));
      }
    }
  ...
  catch (const std::exception& e)
  {
    if (own_error)
    {
      // Simply rethrow our own (directly from this method but not from Fmi::to_string) error
      // directly
      throw;
    }
