...
std::time_t Fmi::last_write_time(const fs::path& path, std::error_code& ec)
{
...
fs::path Fmi::unique_path(const fs::path& model)
{
  ...
  {
    constexpr std::array<char, 63> charset{
        "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"};
    constexpr std::size_t max_index = charset.size() - 1;  // since charset has a null terminator
...
std::optional<std::string> Fmi::lookup_file(const std::string& theFileName)
{
  ...
  {
    std::error_code ec;
    return fs::exists(fn, ec) && (fs::is_regular_file(fn, ec) || fs::is_symlink(fn, ec));
  };
  ...
  {
    static std::vector<std::string> extensions = {".gz",
                                                  ".bz2"
#ifndef WIN32
                                                  ,
                                                  ".xz",
