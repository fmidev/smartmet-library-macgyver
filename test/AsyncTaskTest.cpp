...
namespace AsyncTaskTest {
  ...
  void task_throws_not_std_exception()
  {
    ...
    if (task.get_status() != Fmi::AsyncTask::failed) { TEST_FAILED("Not expected test status"); }
    TEST_PASSED();
  }
  ...
  void test_cancel_task()
  {
      ...
      task.wait();
      ptime t2 = microsec_clock::universal_time();
      if (task.get_status() != Fmi::AsyncTask::interrupted) { TEST_FAILED("Not expected test status"); }
      if ((t2-t1).total_milliseconds() > 50) { TEST_FAILED("Canceling task took too long time"); }
      TEST_PASSED();
  }
